...
class Mail extends ex_class
{
    ...
    public function SendMail($emails, $subject, $text, $Attachment)
    {
    ...
    private function GetAttach($content, $nowpart)
    {
        ...
        elseif ($nowpart->encoding == 3)
        {
            //$content = $content;
            //$message2 = imap_base64($message2);
        }
        elseif ($nowpart->encoding == 2)
        {
            $content = imap_binary($content);
        }
        elseif ($nowpart->encoding == 1)
        {
    ...
    public function ReadMail($connect)
    {
        ...
        { //пробегаем по полученному массиву. Каждый элемент массива - новое письмо
    ...
    public function GetMailList($where = [])
    {
        ...
        {
            $attachs = $this->select("mail_attach", "*", [
                "AND" => [
                    "boxid"   => $value["id"],
                    "gkey[>]" => "0",
                    "gkey[<]" => "2",
